...
from DELibrary import Tool, parse_ecm
import os
import re
t = Tool()
...
class EI(Screen):
    ...
    def __evTunedIn(self):
        ...
        if info is not None:
            self.emm_timer.start(10)
            self.ecm_timer.start(10)
        return
        ...
        if info is not None:
            if info.getInfo(iServiceInformation.sIsCrypted):
                if self.count < 4:
                    ...
                    self.ecm_timer.changeInterval(3000)
                ...
                if info != 0:
                    ...
                    if addr != '':
                        if addr.find('127.0.0.1') >= 0 or addr.find('local') >= 0 or addr.find('localhost') >= 0:
                            if using.find('static') >= 0:
                                try:
                                    self['b_emu'].show()
                                except:
                                    self['b_emu_p'].show()

                                self['netcard'].setText(_('Decode: EMU'))
                            else:
                                try:
                                    self['b_card'].show()
                                except:
                                ...
                                self['netcard'].setText(_('Decode: Internal'))
                                if reader != '':
                                    self['netcard'].setText(self['netcard'].getText() + ' from ' + reader)
                                if port != '':
                                    self['netcard'].setText(self['netcard'].getText() + ' Port: ' + str(port))
                            ...
                            self['netcard'].setText(_('Decode: Slot 1'))
                            ...
                            self['netcard'].setText(_('Decode: Slot 2'))
                            ...
                            self['netcard'].setText(_('Decode: USB Reader'))
                        elif using.find('emu') >= 0 or addr.find('emu') >= 0 or reader.find('emu') >= 0:
                            try:
                                self['b_emu'].show()
                            except:
                                self['b_emu_p'].show()
                            ...
                            self['netcard'].setText(_('Decode: Network'))
                            if port != '':
                                self['netcard'].setText(self['netcard'].getText() + ' Source: ' + addr.replace('net (', '') + ':' + port)
                            else:
                                self['netcard'].setText(self['netcard'].getText() + ' Source: ' + addr.replace('net (', ''))
                            if hops > 0:
                ...
                self['netcard'].setText('Decode: FTA')
                try:
                    self['b_fta'].show()
                except:
                    self['b_fta_p'].show()
    ...
    def __evUpdatedInfo(self):
        ...
        if frontendData is not None:
            ...
            if tuner_type == 'DVB-S' or tuner_type == 'DVB-S2':
                ...
                pol = {eDVBFrontendParametersTerrestrial.Modulation_Auto: 'Auto',
                ...
                fec = {eDVBFrontendParametersTerrestrial.FEC_Auto: 'Auto',
