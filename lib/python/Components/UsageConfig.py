...
def InitUsageConfig():
	...
	for i in range(1, 12):
        ...
        config.usage.infobar_permanent = ConfigYesNo(default = False)
        pkgs = os.listdir(resolveFilename(SCOPE_CURRENT_SKIN) + "modules/")
	extraInfobarList = ["skin_infobar-base"]
	for fil in pkgs:
		if fil.find("skin_infobar") != -1 and fil[:-7] not in extraInfobarList:
			extraInfobarList.append(fil[:-7])
	config.usage.infobar_type = ConfigSelection(default = "skin_infobar-base", choices = extraInfobarList)
	...
	config.usage.configselection_showpicons = ConfigYesNo(default=True)
	config.usage.configselection_bigpicons = ConfigYesNo(default=True)
	...
	mountpoints = [(eEnv.resolve('${datadir}/enigma2/picon_50x30/'),eEnv.resolve('${datadir}/enigma2/picon_50x30'))]
	mountpoints += [ (p.mountpoint + '/picon_50x30/', p.mountpoint + '/picon_50x30') for p in harddiskmanager.getConfiguredStorageDevices()]
	config.usage.configselection_piconspath = ConfigSelection(default = eEnv.resolve('${datadir}/enigma2/picon_50x30/'), choices = mountpoints)
