...
def morphRcImagePath(value):
	if rc_model.rcIsDefault() is False:
		if value == '/usr/share/enigma2/skin_default/rc.png' or value == '/usr/share/enigma2/skin_default/rcold.png':
			alue = rc_model.getRcLocation() + 'rc.png'
	return value
...
def loadSingleSkinData(desktop, skin, path_prefix):
	...
	assert skin.tag == "skin", "root element in skin must be 'skin'!"
		...
		if fileExists("/etc/dreamelite-version"):
			oe = "2.2"
		else:
			oe = "2.0"
                #print "Detected OE:", oe
		try:
			f = open("/etc/enigma2/settings","r")
			for line in f.readlines():
				if line.startswith("config.usage.infobar_type"):
					barType = line.split("=")
					print barType
					infobarType = barType[1].strip()
					break
			f.close()
			if not infobarType:
				infobarType = "skin_infobar-base"
		except:
			infobarType = "skin_infobar-base"
			
                # patch check infobar modules and load infobarType
		prefixModules = "%smodules/" % path_prefix
		try:
			pkgs = listdir(prefixModules)
			print "[SKIN] LOADING INFOBAR FOR OE %s" % oe
			#print "[SKIN] Infobar setup type:",infobarType
			for fil in pkgs:
				skinfile = ""
				if fil.endswith("22.xml") and oe == "2.2" and fil.find(infobarType) != -1:
					skinfile = resolveFilename(SCOPE_CURRENT_SKIN) + "modules/%s" % fil
				elif fil.endswith("20.xml") and oe == "2.0" and fil.find(infobarType) != -1:
					skinfile = resolveFilename(SCOPE_CURRENT_SKIN) + "modules/%s" % fil
				if fileExists(skinfile):
					try:
						print "[SKIN] loading infobar module: %s " % skinfile
						loadSkin(skinfile)
						break
					except Exception, err:
						print "[SKIN] module infobar loading error: ", err
		except:
			print "[SKIN] No modules folder found in %s" % path_prefix
	        # end patch
	...
	print "[SKIN] LOADING INCLUDE in %sskin.xml" % path_prefix
	for c in skin.findall("include"):
		filename = c.attrib.get("filename")
		skinfile = ""
		if filename:
			#print "Analizing:", filename
			if not filename.endswith("22.xml") and not filename.endswith("20.xml"):  #load common modules
				skinfile = resolveFilename(SCOPE_CURRENT_SKIN, filename, path_prefix=path_prefix)
			if filename.endswith("22.xml") and oe == "2.2" and filename.find("skin_infobar") != 1:   #load OE2.2 modules
				skinfile = resolveFilename(SCOPE_CURRENT_SKIN, filename, path_prefix=path_prefix)
			elif filename.endswith("20.xml") and oe == "2.0" and filename.find("skin_infobar") != 1:   #load OE2.0 modules
				skinfile = resolveFilename(SCOPE_CURRENT_SKIN, filename, path_prefix=path_prefix)
			if skinfile:
				if not fileExists(skinfile):
					skinfile = resolveFilename(SCOPE_SKIN_IMAGE, filename, path_prefix=path_prefix)
					print "[SKIN] loading include on default path:", skinfile
				try:
					print "[SKIN] loading module: ", skinfile
					loadSkin(skinfile)
				except Exception, err:
					print "[SKIN] module loading error: ", err
		# end patch
